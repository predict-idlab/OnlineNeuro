...
import numpy as np
import pandas as pd
from utils import fetch_data, generate_grids, run_matlab

from common.utils import load_json

# Experiments to evaluate batch_requests.
# Still needs quite some work.
# TODO time this for the evaluation
# TODO time experiments


NUM_POINTS = 3


def main(matlab_call: bool = True, verbose: bool = True, *args, **kwargs) -> None:
    # Load port configuration
    print(f"matlab_call flag: {matlab_call}")
    config_file = Path(__file__).parent / "config.json"
    config = load_json(config_file)
    assert isinstance(config, dict), "Configuration file is not a dictionary"

    # Create a TCP/IP socket
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    if verbose:
        print(
            f"Establishing connection at port {config['ip']} with port {config['port']}"
        )
    ...
    # Respond back
    response = {"message": "Hello from Python", "randomNumber": data["dummyNumber"]}
    ...
    response = {
        "message": "All data points to query",
    ...
    exp_config_file = (
        Path("config_experiments") / f"experiment_{exp_config['name']}.json"
    )
    carried_exp = load_json(exp_config_file)

    default_config_file = (
        Path("config_experiments") / f"parameter_spec_{exp_config['name']}.json"
    )
    ...
    save_path = (
        Path(f"{config['benchmark_path']}")
        / f"{exp_config['name']}"
        / f"fib_dia_{diam}mm_dip_dis{dip_dis}mm_{NUM_POINTS}/"
    )
