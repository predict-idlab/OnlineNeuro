function feat_struct = preprocess_struct(feat_struct)
    ...
    for i = 1:numel(funTypes)
        ...
        if ismember(currentType, {'single_pulse','double_pulse'})
           ...
           field_name = sprintf('pulse_parameters_%d_I',i);
           feat_struct.I(i) = feat_struct.(field_name);
           field_name = sprintf('pulse_parameters_%d_pulse_dur',i);
        ...
        elseif ismember(currentType, {'pulse_ramp'})
            ...
            delay_name = sprintf('pulse_parameters_%d_delay',i);
            amplitude_name = sprintf('pulse_parameters_%d_amplitude',i);
            pulse_width_name = sprintf('pulse_parameters_%d_pulse_width',i);
            interphase_gap_name = sprintf('pulse_parameters_%d_interphase_gap',i);
            decay_width_name = sprintf('pulse_parameters_%d_decay_width',i);
            k_name = sprintf('pulse_parameters_%d_k',i);
            ramp_width_name = sprintf('pulse_parameters_%d_ramp_width',i);
